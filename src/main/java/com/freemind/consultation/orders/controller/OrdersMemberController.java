...
public class OrdersMemberController {
	...
	// 濾掉「這位會員自己」已預約(待確認0/已確認1)的小時，避免看到自己的訂單又跳重複
	private void removeMyBookedHours(List<Integer> hours, Integer memberId, Integer pid, LocalDate date) {
		if (memberId == null) return;
		java.util.Set<Integer> mine = ordersSvc.getByMemberId(memberId).stream()
				.filter(o -> o.getPsychologist() != null
						&& o.getPsychologist().getPsychId().equals(pid)
						&& o.getConsStart() != null
						&& o.getConsStart().toLocalDate().equals(date)
						&& (o.getOrderStatus() == 0 || o.getOrderStatus() == 1))
				.map(o -> o.getConsStart().getHour())
				.collect(java.util.stream.Collectors.toSet());
		hours.removeIf(mine::contains);
	}
	...
	public String bookLookupFromCalendar(@RequestParam("psychId") String psychId,
		...
		Integer myId = (prinUserDetails != null) ? prinUserDetails.getMember().getMemberId() : null;
		removeMyBookedHours(availableHours, myId, pid, date);   // ★ 濾掉自己已約的
	...
	public String bookSubmit(@RequestParam("timeslotId") String timeslotId, @RequestParam(value = "hour", required = false) String hourStr,
		...
		if (hourStr == null || hourStr.isBlank()) {
			...
			List<Integer> ah0 = computeAvailableHours(slot0, slot0.getSlotDate());
			removeMyBookedHours(ah0, memberId, Integer.valueOf(psychId), slot0.getSlotDate());
			model.addAttribute("availableHours", ah0);
		...
		if (consStart.isBefore(java.time.LocalDateTime.now())) {
			...
			List<Integer> ah = computeAvailableHours(slot, slot.getSlotDate());
			removeMyBookedHours(ah, memberId, pid, slot.getSlotDate());
			model.addAttribute("availableHours", ah);
			model.addAttribute("psychologist", psychologist);
