...
public class AdminController {
	...
	/*
	 * 顯示管理員照片；listAllAdmin.html、listOneAdmin.html、update_admin_input.html 用
	 */
	@GetMapping("DBGifReader")
	public void dBGifReader(@RequestParam("adminId") String adminId,
	        HttpServletRequest req, HttpServletResponse res) throws IOException {
	    res.setContentType("image/gif");
	    ServletOutputStream out = res.getOutputStream();
	    try {
	        out.write(adminSvc.getOneAdmin(Integer.valueOf(adminId)).getProfilePic());
	    } catch (Exception e) {
	 ...
	 */
	@PostMapping("insert")
	public String insert(@Valid @ModelAttribute("adminVO") Admin adminVO, BindingResult result, ModelMap model,
	 ...
	 */
	@PostMapping("update")
	public String update(@Valid @ModelAttribute("adminVO") Admin adminVO, BindingResult result, ModelMap model,
			@RequestParam("profilePic") MultipartFile[] parts) throws IOException {
		...
		// 權限不在修改表單上，繫結出來的 adminVO 權限集合是空的；
		// 自資料庫取回舊權限重新塞回，防止 save 時把 admin_permissions 的關聯整批清空
